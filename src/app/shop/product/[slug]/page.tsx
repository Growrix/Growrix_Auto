...
import { getCategoryBySlug, getProductBySlug, productToCurrency, products } from "@/data/catalog";
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  return (
    <div className="bg-white">
      <Breadcrumbs items={["Home", "Shop", category?.label ?? "Category", product.title]} />
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 pb-16 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-10">
        ...
        <div>
          ...
          <div className="mt-4 flex items-center gap-3 text-[22px] font-bold">
            ...
            {product.oldPrice ? (
              <span className="text-[18px] text-[#999] line-through">{productToCurrency(product.oldPrice)}</span>
            ) : null}
          </div>
