...
export default async function ProductPage({ params }: ProductPageProps) {
  const { formatPrice, language, t } = await getServerPreferences();
  ...
  const localizedCategoryLabel = category ? getCategoryLabel(category, language) : t("common.category");
  const localizedTitle = getProductTitle(product, language);
  const localizedExcerpt = getProductExcerpt(product, language);
  const featureItems = [
    t("product.featureCompatibility", { category: localizedCategoryLabel }),
  ...
  return (
    <div className="bg-white">
      <Breadcrumbs items={["Home", "Shop", localizedCategoryLabel, localizedTitle]} />
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 pb-16 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-10">
        <div className="relative h-120 overflow-hidden border border-[#eee] bg-[#fafafa]">
          <Image src={product.image} alt={localizedTitle} fill sizes="(min-width: 1024px) 50vw, 100vw" className="object-contain" />
        </div>

        <div>
          <h1 className="text-[30px] font-black uppercase text-[#222]">{localizedTitle}</h1>
          ...
          <p className="mt-6 text-[15px] leading-7 text-[#666]">{localizedExcerpt}</p>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <article className="border border-[#ececec] bg-[#fafafa] p-6 lg:p-8">
            ...
            <div className="mt-4 space-y-4 text-[15px] leading-8 text-[#666]">
              <p>{t("product.descriptionBody1", { title: localizedTitle })}</p>
              <p>{t("product.descriptionBody2", { category: localizedCategoryLabel })}</p>
            </div>
          </article>
