...
export default function BlogSection() {
  const { selectedLanguage, t } = useUtility();
    ...
    <section className="bg-white py-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {blogPosts.map((post) => {
            const localizedPost = getLocalizedBlogPost(post, selectedLanguage);

            return (
              <article key={post.slug} className="group">
                <div className="relative h-55 overflow-hidden">
                  <Image src={post.image} alt={localizedPost.title} fill sizes="(min-width: 1024px) 33vw, 100vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
                </div>
                <p className="mt-3 text-[12px] text-[#999]">{post.date}</p>
                <h3 className="mt-2 text-[16px] font-bold uppercase text-[#111]">{localizedPost.title}</h3>
                <p className="mt-2 text-[13px] leading-6 text-[#666]">{localizedPost.excerpt}</p>
                <Link
