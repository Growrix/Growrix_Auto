...
import { getCategoryLabel } from "@/data/catalog";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          {homeCategories.map((item) => {
            const localizedLabel = getCategoryLabel(item, selectedLanguage);
            ...
            return (
              ...
              >
                <div className="flex h-24 w-24 items-center justify-center rounded-full border border-[#ddd] bg-white shadow-[0_6px_18px_rgba(0,0,0,0.06)] transition-all duration-300 group-hover:-translate-y-1 group-hover:border-[#ff3434] group-hover:shadow-xl">
                  <Image src={item.image} alt={localizedLabel} width={72} height={72} className="h-16 w-16 rounded-full object-cover" />
                </div>
                <p className="max-w-30 text-[12px] font-semibold uppercase leading-5 text-[#222] transition-colors duration-300 group-hover:text-[#ff3434]">{localizedLabel}</p>
              </Link>
            );
          })}
        </div>
