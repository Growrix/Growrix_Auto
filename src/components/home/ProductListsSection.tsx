...
import { getProductTitle, getProductsBySlugs } from "@/data/catalog";
          ...
          <div key={column.title}>
            ...
            <div className="space-y-5">
              {getProductsBySlugs(column.slugs).map((item) => {
                const localizedTitle = getProductTitle(item, selectedLanguage);
                ...
                return (
                  <Link key={item.slug} href={productPath(item.slug)} className="group flex gap-4 border-b border-[#ececec] pb-4 transition-colors duration-300 last:border-0 hover:border-[#ffb3b3]">
                    <div className="relative h-16 w-16 shrink-0">
                      <Image src={item.image} alt={localizedTitle} fill sizes="64px" className="object-contain transition-transform duration-300 group-hover:scale-105" />
                    </div>
                    <div>
                      <p className="text-[13px] font-medium uppercase leading-5 text-[#222] transition-colors duration-300 group-hover:text-[#ff3434]">{localizedTitle}</p>
